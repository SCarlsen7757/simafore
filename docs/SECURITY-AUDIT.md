...
- **#8:** Primary links and QR codes derive from validated SSA IDs, including cached records. Regression tests cover foreign and deceptive destinations, HTTP, credentials, mismatched paths, invalid IDs and legitimate external remediation URLs. Feed configuration now requires credential-free HTTPS.

Final local verification: `npm run check` passed all 29 tests, type checking, lint and formatting; the production build passed. Seven browser scenes, rotation, offline continuity and refresh recovery passed using installed Chrome. The amd64 Docker build and disposable smoke test passed, including non-root execution, packaged worker processing, schema 2 API, static font and bind-mount persistence. The first smoke run exposed inherited CLI flags in the worker launcher; explicit worker flags fixed that failure before the passing rerun. Minimum-Node and arm64 validation remain CI checks; live tunnel/LAN reachability and vulnerability advisory scans were not performed in this implementation run.
